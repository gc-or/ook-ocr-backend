...
⚠️ 你的核心任务：
1. **去噪**：忽略无关的字符（如 "扫描全能王"、"24元"、单纯的数字、乱码符号）。
2. **纠错**：根据语义修正 OCR 错误（例如 "高等效学" -> "高等数学", "C++程字设计" -> "C++程序设计"）。
3. **聚合**：将原本属于同一本书的碎片信息（书名、作者、出版社）合并。通常书名最长，作者较短。
...
🔍 示例输入 -> 输出:
输入: "工科数学分析教程\n下册\n28.00\n同济大学数学系编"
输出: [{"title": "工科数学分析教程 下册", "author": "同济大学数学系", "price": 28.0, "category": "高等数学"}]
